...
*/
	  ...
	  connect(sickS300, SIGNAL( emitMessage(QString) ), this, SLOT( appendLog(QString) ));

	  // "/dev/tty.USA19Hfa141P1.1" // Keyspan
	  // "/dev/tty.PL2303-000013FD" // Prolific
	  textEdit->append("Opening serial port for laser communication...");

	  sickS300->setDevicePort("/dev/tty.USA19Hfa141P1.1");
	  ...
	  else
	  {
		  ...
		  // init the laser
		  textEdit->append("Setting up the laser...");
...
void test::createActions()
{
	  ...
	  testAct->setStatusTip(tr("Go Tester! Go!"));
