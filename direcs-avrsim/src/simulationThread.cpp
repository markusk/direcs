...
/* infrared Sensors temporarily removed from robot!!
			...
			if (interface1->sendChar(READ_SENSOR_1) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR s1 sending to serial port (SimulationThread)");
				return;
			}
			...
			if (interface1->sendChar(READ_SENSOR_2) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR s2 sending to serial port (SimulationThread)");
				return;
			}
			...
			if (interface1->sendChar(READ_SENSOR_3) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR s3 sending to serial port (SimulationThread)");
				return;
			}
			...
			if (interface1->sendChar(READ_SENSOR_4) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR s4 sending to serial port (SimulationThread)");
				return;
			}
			...
			if (interface1->sendChar(READ_SENSOR_5) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR s5 sending to serial port (SimulationThread)");
				return;
			}
			...
			if (interface1->sendChar(READ_SENSOR_6) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR s6 sending to serial port (SimulationThread)");
				return;
			}
...
sensor 7 and 8 now voltage sensors!!
			...
			if (interface1->sendChar(READ_SENSOR_7) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR s7 sending to serial port (SimulationThread)");
				return;
			}
			...
			if (interface1->sendChar(READ_SENSOR_8) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR s8 sending to serial port (SimulationThread)");
				return;
			}
...
sensor 7 and 8 now voltage sensors!!
			...
			if (interface1->sendChar(READ_SENSOR_16) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR s16 sending to serial port (SimulationThread)");
				return;
			}
...
*/
			...
			if (interface1->sendChar(READ_SENSOR_7) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR reading voltage sensor 2 [SimulationThread]");
				return;
			}
			...
			if (interface1->sendChar(READ_MOTOR_SENSOR1) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR reading motor sensor 1 [SimulationThread]");
				return;
			}
			...
			if (interface1->sendChar(READ_MOTOR_SENSOR2) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR reading motor sensor 2 [SimulationThread]");
				return;
			}
			...
			if (interface1->sendChar(READ_MOTOR_DISTANCE1) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR md1 sending to serial port (SimulationThread)");
				return;
			}
			...
			if (interface1->sendChar(READ_MOTOR_DISTANCE2) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR md2 sending to serial port (SimulationThread)");
				return;
			}
...
/* contacts temporarily removed from robot!!
			...
			if (interface1->sendChar(READ_CONTACT1) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR c1 sending to serial port (SimulationThread)");
				return;
			}
			...
			if (interface1->sendChar(READ_CONTACT2) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR c2 sending to serial port (SimulationThread)");
				return;
			}
			...
			if (interface1->sendChar(READ_CONTACT3) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR c3 sending to serial port (SimulationThread)");
				return;
			}
			...
			if (interface1->sendChar(READ_CONTACT4) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR c4 sending to serial port (SimulationThread)");
				return;
			}
...
contacts temporarily removed from robot!! */
			...
			if (interface1->sendChar(READ_AXIS_X) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR x sending to serial port (SimulationThread)");
				return;
			}
...
// 			qDebug("From Atmel=%d / xAxis=%f", value, xAxis);
			...
			if (interface1->sendChar(READ_AXIS_Y) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR y sending to serial port (SimulationThread)");
				return;
			}
			...
			if (interface1->sendChar(READ_AXIS_Z) == false)
			{
				// Unlock the mutex.
				mutex->unlock();
				qDebug("ERROR z sending to serial port (SimulationThread)");
				return;
			}
...
int SimulationThread::convertToDistance(int sensorValue)
{
...
int SimulationThread::convertToSensorValue(int distance)
{
	unsigned char calibrationValue = 8;
...
int SimulationThread::getMAmpere(int sensor)
{
...
float SimulationThread::getVoltage(int sensor)
{
...
int SimulationThread::getContactValue(int contact)
{
...
int SimulationThread::getIrSensorValue(int sensor)
{
...
int SimulationThread::getDistance(int sensor)
{
	int distance = 0;
...
int SimulationThread::getDrivenDistance(int sensor)
{
...
void SimulationThread::resetDrivenDistance(int sensor)
{
	if ((sensor < MOTORSENSOR1) || (sensor > DRIVENDISTANCEARRAYSIZE-1))
	{
		qDebug("ERROR: wrong motor sensor");
...
*/
int SimulationThread::getUsSensorValue(int sensor)
{
...
int SimulationThread::getMotorSensorValue(int sensor)
{
	//
...
int SimulationThread::getCompassValue(unsigned char axis)
{
	switch (axis)
	{
		case READ_AXIS_X:
...
float SimulationThread::convertToDegree(int sensorValue)
{
...
float SimulationThread::convertToVolt(int sensor)
{
	if ((sensor < VOLTAGESENSOR1) || (sensor > VOLTAGESENSOR2))
	{
